...
if ( ! class_exists( 'TMCECF_Plugin_Activation' ) ) {
    /**
     ...
     * @author  Gary Jones <gamajo.com>
     */
    class TMCECF_Plugin_Activation {
         ...
         * @var string
         */
        public $menu = 'tmcecf-install-plugins';
         ...
         * does the tmcecf_init action hook, and hooks in the interactions to init.
         *
         * @since 1.0.0
         *
         * @see TMCECF_Plugin_Activation::init()
         */
        public function __construct() {
            ...
            $this->strings = array(
                'page_title'                     => __( 'Install Required Plugins', 'tmcecf' ),
                'menu_title'                     => __( 'Install Plugins', 'tmcecf' ),
                'installing'                     => __( 'Installing Plugin: %s', 'tmcecf' ),
                'oops'                           => __( 'Something went wrong.', 'tmcecf' ),
                ...
                'return'                         => __( 'Return to Required Plugins Installer', 'tmcecf' ),
                'dashboard'                      => __( 'Return to the dashboard', 'tmcecf' ),
                'plugin_activated'               => __( 'Plugin activated successfully.', 'tmcecf' ),
                'activated_successfully'         => __( 'The following plugin was activated successfully:', 'tmcecf' ),
                'complete'                       => __( 'All plugins installed and activated successfully. %1$s', 'tmcecf' ),
                'dismiss'                        => __( 'Dismiss this notice', 'tmcecf' ),
            );
            ...
            do_action_ref_array( 'tmcecf_init', array( $this ) );
         ...
         * @since 2.0.0
         *
         * @see TMCECF_Plugin_Activation::admin_menu()
         * @see TMCECF_Plugin_Activation::notices()
         * @see TMCECF_Plugin_Activation::styles()
         */
        public function init() {

            do_action( 'tmcecf_register' );
        ...
        public function thickbox() {
            ...
            if ( ! get_user_meta( get_current_user_id(), 'tmcecf_dismissed_notice', true ) ) {
                add_thickbox();
            }
         ...
         *
         * @see TMCECF_Plugin_Activation::init()
         * @see TMCECF_Plugin_Activation::install_plugins_page()
         */
        ...
        public function install_plugins_page() {
            ...
            $plugin_table = new  TMCECF_List_Table;

            // Return early if processing a plugin installation action.
            if ( isset( $_POST['action'] ) && 'tmcecf-bulk-install' == $_POST['action'] && $plugin_table->process_bulk_actions() || $this->do_plugin_install() ) {
                return;
            }

            ?>
            <div class="tmcecf wrap">
                ...
                <form id="tmcecf-plugins" action="" method="post">
                    <input type="hidden" name="tmcecf-page" value="<?php echo $this->menu; ?>" />
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['tmcecf-install'] ) && 'install-plugin' == $_GET['tmcecf-install'] ) ) {
                check_admin_referer( 'tmcecf-install' );
                ...
                $fields = array( 'tmcecf-install' ); // Extra fields to pass to WP_Filesystem.
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . network_admin_url() . '" title="' . __( 'Return to the Dashboard', 'tmcecf' ) . '">' . __( 'Return to the Dashboard', 'tmcecf' ) . '</a>' ) . '</p>';
            ...
            elseif ( isset( $_GET['plugin'] ) && ( isset( $_GET['tmcecf-activate'] ) && 'activate-plugin' == $_GET['tmcecf-activate'] ) ) {
                check_admin_referer( 'tmcecf-activate', 'tmcecf-activate-nonce' );
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                foreach ( $message as $type => $plugin_groups ) {
                    ...
                    $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'tmcecf' ), $imploded, $count ) . '</p>';
                }

                // Setup variables to determine if action links are needed.
                $show_install_link  = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, network_admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'tmcecf' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . add_query_arg( 'page', $this->menu, network_admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'tmcecf' ) . '</a>'  : '';

                // Define all of the action links.
                $action_links = apply_filters(
                    'tmcecf_notice_action_links',
                    array(
                        'install'  => ( current_user_can( 'install_plugins' ) )  ? $show_install_link  : '',
                        'activate' => ( current_user_can( 'activate_plugins' ) ) ? $show_activate_link : '',
                        'dismiss'  => $this->dismissable ? '<a class="dismiss-notice" href="' . add_query_arg( 'tmcecf-dismiss', 'dismiss_admin_notices' ) . '" target="_parent">' . $this->strings['dismiss'] . '</a>' : '',
                    )
                );
                ...
                if ( ! empty( $this->strings['nag_type'] ) ) {
                    add_settings_error( 'tmcecf', 'tmcecf', $rendered, sanitize_html_class( strtolower( $this->strings['nag_type'] ) ) );
                } else {
                    add_settings_error( 'tmcecf', 'tmcecf', $rendered, $nag_class );
                }
            }

            // Admin options pages already output settings_errors, so this is to avoid duplication.
            if ( 'options-general' !== $current_screen->parent_base ) {
                settings_errors( 'tmcecf' );
            }
        ...
        public function dismiss() {
            ...
            if ( isset( $_GET['tmcecf-dismiss'] ) ) {
                update_user_meta( get_current_user_id(), 'tmcecf_dismissed_notice', 1 );
            }
         ...
         */
        protected function is_tmcecf_page() {
        ...
        public function update_dismiss() {
            ...
            delete_user_meta( get_current_user_id(), 'tmcecf_dismissed_notice' );
        ...
        public static function get_instance() {
            ...
            if ( ! isset( self::$instance ) && ! ( self::$instance instanceof TMCECF_Plugin_Activation ) ) {
                self::$instance = new TMCECF_Plugin_Activation();
            }
    ...
    $tmcecf = TMCECF_Plugin_Activation::get_instance();

}

if ( ! function_exists( 'tmcecf' ) ) {
    /**
     ...
     */
    function tmcecf( $plugins, $config = array() ) {

        foreach ( $plugins as $plugin ) {
            TMCECF_Plugin_Activation::$instance->register( $plugin );
        }

        if ( $config ) {
            TMCECF_Plugin_Activation::$instance->config( $config );
        }
...
if ( ! class_exists( 'TMCECF_List_Table' ) ) {
    /**
     ...
     */
    class  TMCECF_List_Table extends WP_List_Table {
        ...
        protected function _gather_plugin_data() {
            ...
            TMCECF_Plugin_Activation::$instance->admin_init();
            TMCECF_Plugin_Activation::$instance->thickbox();
            ...
            foreach ( TMCECF_Plugin_Activation::$instance->plugins as $plugin ) {
                if ( is_plugin_active( $plugin['file_path'] ) ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = __( 'Private Repository', 'tmcecf' );
                    // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = __( 'Pre-Packaged', 'tmcecf' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = __( 'WordPress Repository', 'tmcecf' );
                }

                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'tmcecf' ) : __( 'Recommended', 'tmcecf' );

                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'tmcecf' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'tmcecf' ) );
                }
        ...
        protected function _get_plugin_data_from_name( $name, $data = 'slug' ) {
            ...
            foreach ( TMCECF_Plugin_Activation::$instance->plugins as $plugin => $values ) {
                if ( $name == $values['name'] && isset( $values[$data] ) ) {
                    return $values[$data];
                }
            }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . __( 'Install', 'tmcecf' ) . ' %2$s">' . __( 'Install', 'tmcecf' ) . '</a>',
                        wp_nonce_url(
                            add_query_arg(
                                array(
                                    'page'          => TMCECF_Plugin_Activation::$instance->menu,
                                    'plugin'        => $item['slug'],
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . __( 'Activate', 'tmcecf' ) . ' %2$s">' . __( 'Activate', 'tmcecf' ) . '</a>',
                        add_query_arg(
                            array(
                                'page'                 => TMCECF_Plugin_Activation::$instance->menu,
                                'plugin'               => $item['slug'],
                                'plugin_name'          => $item['sanitized_plugin'],
                                'plugin_source'        => $item['url'],
                                'tmcecf-activate'       => 'activate-plugin',
                                'tmcecf-activate-nonce' => wp_create_nonce( 'tmcecf-activate' ),
                            ),
                            network_admin_url( 'themes.php' )
                        ),
                        $item['sanitized_plugin']
        ...
        public function no_items() {
            ...
            printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'tmcecf' ), network_admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_columns() {
            ...
            $columns = array(
                ...
                'plugin' => __( 'Plugin', 'tmcecf' ),
                'source' => __( 'Source', 'tmcecf' ),
                'type'   => __( 'Type', 'tmcecf' ),
                'status' => __( 'Status', 'tmcecf' )
            );
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'tmcecf-bulk-install'  => __( 'Install', 'tmcecf' ),
                'tmcecf-bulk-activate' => __( 'Activate', 'tmcecf' ),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ( 'tmcecf-bulk-install' === $this->current_action() ) {
                ...
                $url = wp_nonce_url(
                    add_query_arg(
                        array(
                            'page'          => TMCECF_Plugin_Activation::$instance->menu,
                            'tmcecf-action'  => 'install-selected',
                ...
                // Loop through each plugin to install and try to grab information from WordPress API, if not create 'tmcecf-empty' scalar.
                $i = 0;
                foreach ( $plugin_installs as $plugin ) {
                    $api[$i] = plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) ? plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) : (object) $api[$i] = 'tmcecf-empty';
                    $i++;
                }

                if ( is_wp_error( $api ) ) {
                    wp_die( TMCECF_Plugin_Activation::$instance->strings['oops'] . var_dump( $api ) );
                }
                ...
                $url   = add_query_arg( array( 'page' => TMCECF_Plugin_Activation::$instance->menu ), network_admin_url( 'themes.php' ) );
                $nonce = 'bulk-plugins';
                $names = $plugin_names;

                // Create a new instance of TGM_Bulk_Installer.
                $installer = new  TMCECF_Bulk_Installer( $skin = new TMCECF_Bulk_Installer_Skin( compact( 'url', 'nonce', 'names' ) ) );

                // Wrap the install process with the appropriate HTML.
                echo '<div class="tmcecf wrap">';
            ...
            if ( 'tmcecf-bulk-activate' === $this->current_action() ) {
                check_admin_referer( 'bulk-' . $this->_args['plural'] );
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'tmcecf' ), $imploded );
                }
 ...
 * @since 2.2.0
 */
if ( ! class_exists( 'WP_Upgrader' ) && ( isset( $_GET['page'] ) && TMCECF_Plugin_Activation::$instance->menu === $_GET['page'] ) ) {
    require_once ABSPATH . 'wp-admin/includes/class-wp-upgrader.php';

    if ( ! class_exists( 'TMCECF_Bulk_Installer' ) ) {
        /**
         ...
         * @author  Gary Jones <gamajo.com>
         */
        class TMCECF_Bulk_Installer extends WP_Upgrader {
            ...
            public function bulk_install( $packages ) {
                ...
                if ( TMCECF_Plugin_Activation::$instance->is_automatic ) {
            ...
            public function run( $options ) {
                ...
                if ( TMCECF_Plugin_Activation::$instance->is_automatic ) {
                    ...
                    TMCECF_Plugin_Activation::$instance->populate_file_path();
            ...
            public function install_strings() {
                ...
                $this->strings['no_package']          = __( 'Install package not available.', 'tmcecf' );
                $this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'tmcecf' );
                $this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'tmcecf' );
                $this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'tmcecf' );
                $this->strings['process_failed']      = __( 'Plugin install failed.', 'tmcecf' );
                $this->strings['process_success']     = __( 'Plugin installed successfully.', 'tmcecf' );
            ...
            public function activate_strings() {
                ...
                $this->strings['activation_failed']  = __( 'Plugin activation failed.', 'tmcecf' );
                $this->strings['activation_success'] = __( 'Plugin activated successfully.', 'tmcecf' );
    ...
    if ( ! class_exists( 'TMCECF_Bulk_Installer_Skin' ) ) {
        /**
         ...
         * @author  Gary Jones <gamajo.com>
         */
        class TMCECF_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
            ...
            public function add_strings() {
                ...
                if ( TMCECF_Plugin_Activation::$instance->is_automatic ) {
                    $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'tmcecf' );
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'tmcecf' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'tmcecf' ) . '</span><span class="hidden">' . __( 'Hide Details', 'tmcecf' ) . '</span>.</a>';
                    $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'tmcecf' );
                    $this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'tmcecf' );
                }
                // Default installation strings.
                else {
                    $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'tmcecf' );
                    $this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'tmcecf' );
                    $this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'tmcecf' );
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'tmcecf' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'tmcecf' ) . '</span><span class="hidden">' . __( 'Hide Details', 'tmcecf' ) . '</span>.</a>';
                    $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'tmcecf' );
                    $this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'tmcecf' );
                }
            ...
            public function bulk_footer() {
                ...
                foreach ( TMCECF_Plugin_Activation::$instance->plugins as $plugin ) {
                    if ( ! is_plugin_active( $plugin['file_path'] ) ) {
                        echo '<p><a href="' . add_query_arg( 'page', TMCECF_Plugin_Activation::$instance->menu, network_admin_url( 'themes.php' ) ) . '" title="' . esc_attr( TMCECF_Plugin_Activation::$instance->strings['return'] ) . '" target="_parent">' . TMCECF_Plugin_Activation::$instance->strings['return'] . '</a></p>';
                        $complete[] = $plugin;
                        break;
                    }
                    // Nothing to store.
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( TMCECF_Plugin_Activation::$instance->strings['complete'], '<a href="' . network_admin_url() . '" title="' . __( 'Return to the Dashboard', 'tmcecf' ) . '">' . __( 'Return to the Dashboard', 'tmcecf' ) . '</a>' ) . '</p>';
                    echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                }
